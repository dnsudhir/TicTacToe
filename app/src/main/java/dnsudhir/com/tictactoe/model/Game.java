...
public class Game {
  ...
  public boolean hasSameDiagonalCells() {
    try {
      return areEqual(cells[0][0], cells[1][1], cells[2][2]) || areEqual(cells[0][2], cells[1][1],
          cells[2][0]);
    } catch (NullPointerException e) {
      Log.e(TAG, e.getMessage());
      return false;
  ...
  private boolean areEqual(Cell... cells) {
    ...
    Cell comparsionBase = cells[0];
    for (int i = 0; i < cells.length; i++) {
      if (!comparsionBase.player.value.equals(cells[i].player.value)) {
        return false;
      }
    }
    return true;
